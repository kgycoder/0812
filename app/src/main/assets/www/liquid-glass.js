...
(function () {
    ...
    function applyContinuousCorner(el, radius, exponent, maskPathId, maskId) {
        if (!el) return;
        // transform(scale-in 애니메이션 등)에 영향받지 않는 레이아웃 크기를 사용해야
        // 애니메이션 도중에도 항상 정확한 최종 크기로 마스크 경로를 그릴 수 있다.
        const w = el.offsetWidth, h = el.offsetHeight;
        if (!w || !h) return;
        const rad = Math.min(radius != null ? radius : 24, w / 2, h / 2);
        const d = squirclePath(w, h, rad, exponent);
        // border-radius는 mask 로딩 전/실패 시를 대비한 폴백일 뿐이다.
        el.style.borderRadius = rad + "px";
        if (!maskPathId) return;
        const path = $(maskPathId);
        const mask = maskId ? $(maskId) : null;
        if (!path) return;
    ...
    /* ---------- 1d. 안드로이드 키보드 대응 (VisualViewport) ----------
       소프트 키보드가 올라오면 실제로 보이는 뷰포트(window.visualViewport)가
       줄어드는데, 오버레이가 100dvh 고정 기준으로 가운데 정렬되어 있으면
       그 중심이 "줄어들기 전" 화면 기준으로 계산되어 박스가 키보드 밑으로
       가려지거나 하단이 잘려 보인다. visualViewport의 실측 높이/오프셋을
       CSS 변수로 흘려보내, 오버레이가 항상 키보드 위의 "실제 보이는 영역"
       안에서 가운데 정렬 + 높이 제한되도록 한다. */
    function setupKeyboardAwareOverlays() {
        ...
        function update() {
            if (vv) {
                ...
                root.style.setProperty("--vv-h", innerHeight + "px");
                root.style.setProperty("--vv-top", "0px");
            }
            // CSS 변수 자체는 매 프레임 갱신해 transition으로 부드럽게 따라가지만,
            // 굴절 맵/마스크 재계산(캔버스 생성 등 무거운 작업)은 키보드 애니메이션이
            // "멈춘 뒤"에 한 번만 실행한다 — 안 그러면 슬라이드 도중 계속 재계산되어
            // 박스가 계속 크기를 바꾸는 것처럼 보이는 문제(끊김/떨림)가 생긴다.
            clearTimeout(settleTimer);
            settleTimer = setTimeout(applyPlaylistOverlayGlass, 320);
        }
        let raf = null;
    ...
    function applyPlaylistOverlayGlass() {
        ...
        if (ctx && ctx.classList.contains("on")) {
            ...
            applyContinuousCorner(ctx, 20, EXP, "#plCtxMaskPath", "#plCtxMask");
        }
        ...
        if (dialog && dialog.closest("#pl-dialog-overlay")?.classList.contains("on")) {
            ...
            applyContinuousCorner(dialog, 28, EXP, "#plDialogMaskPath", "#plDialogMask");
        }
        ...
        if (confirm && confirm.closest("#pl-confirm-overlay")?.classList.contains("on")) {
            ...
            applyContinuousCorner(confirm, 28, EXP, "#plConfirmMaskPath", "#plConfirmMask");
        }
        ...
        if (addModal && addModal.closest("#pl-add-modal-overlay")?.classList.contains("on")) {
            ...
            applyContinuousCorner(addModal, 30, EXP, "#plAddModalMaskPath", "#plAddModalMask");
        }
    }
